...
// 
// linkurl [string] (default: set to »original image« )
...
function smarty_function_thumb_imp($params, &$smarty) {/*{{{*/
	...
	if (empty($params['cache'])) {
        ...
        if ((substr($_CONFIG['cache'], -1)) != '/') $_CONFIG['cache'] .= "/";
	...
	if (empty($params['getcachefilename'])) $params['getcachefilename'] = false;
    ...
    if (isset($params['width']) && is_numeric($params['width'])) $_DST['width'] = $params['width'];
    else {
        if (empty($params['height'])) $params['height'] = 100;
        $_DST['width'] = round($params['height']/($_SRC['height']/$_SRC['width']));
    }

	if (isset($params['height']) && is_numeric($params['height'])) $_DST['height']	= $params['height'];
    else {
	...
	if (empty($params['html'])) $_RETURN['img'] = '<img src="'.$_DST['file'].'" '.$_DST['string'].' alt="" title="" />';
...
/*}}}*/
	...
	if ($_DST['width']*4 < $_SRC['width'] AND $_DST['height']*4 < $_SRC['height']) {
        ...
        if (!isset($_DST['offset_w'])) { $_DST['offset_w'] = 0; }
        if (!isset($_DST['offset_h'])) { $_DST['offset_h'] = 0; }
    ...
    if ($_SRC['image']) {
        ...
        if (isset($params['sharpen']) && ($params['sharpen'] != "false")) $_DST['image'] = UnsharpMask($_DST['image'],80,.5,3);
	}
